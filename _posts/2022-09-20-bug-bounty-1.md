...
excerpt: "Partial disclosure of a bug bounty report: reflected XSS bypassing an HTML tag removal sanitization."
date: 2021-09-20
classes: wide
header:
...
> :warning: <span style="color:red">This bug was found in a private bug bounty program in which it is not allowed to publish the vulnerabilities found. So this is a partial disclosure, only the essential technical details are exposed.</span>

In this post I am going to show my first relevant report in a bug bounty program: a reflected XSS bypassing an HTML tag removal sanitization. This was not my first report but it was the first one where using pentesting tools was not enough and I had to think outside the box.
...
So far, mission accomplished. Then I tried to inject some javascript code through a request of the form
...
# Get the boolean variable based sanitization of the input
...
# Get the stack based sanitization of the input
def get_stack_based_sanitization(input):
...
if __name__ == '__main__':
    ...
    for input in input_list:
        ...
        stack_based_sanitization = get_stack_based_sanitization(input)
        ...
        output = output + "Stack based sanitization  ->  " + stack_based_sanitization + 2*"\n"
