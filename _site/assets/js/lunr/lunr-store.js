var store = [{
        "title": "eJPT certification review",
        "excerpt":"Summary 1. What is eJPT? 2. What will you learn and what previous knowledge do you need? 3. Is it useful for the curriculum? 4. Is it affordable? 5. How is the content? 5.1. Penetration Testing Prerequisites 5.2. Penetration Testing: Preliminary Skills &amp; Programming 5.3. Penetration Testing Basics 5.4. eJPT...","categories": ["Certification"],
        "tags": ["eLearnSecurity","eJPT","pentesting"],
        "url": "https://hackcommander.io/ejpt-certification-review/",
        "teaser":"https://hackcommander.io/assets/images/2022-09-01-ejpt-certification-review/ejpt-logo.png"},{
        "title": "Reflected XSS bypassing an HTML tag removal sanitization",
        "excerpt":"Summary 1. Asset discovery 2. Vulnerability discovery 3. Vulnerability exploitation 3.1. Steps of exploitation 3.2. Why does the payload work? 4. Report resolution :warning: This bug was found in a private bug bounty program in which it is not allowed to publish the vulnerabilities found. So this is a partial...","categories": ["Bug Bounty"],
        "tags": ["web pentesting","OSINT","amass","httpx","gau","kxss","XSS","python","bypass","sanitization"],
        "url": "https://hackcommander.io/bug-bounty-1/",
        "teaser":"https://hackcommander.io/assets/images/general/bug-bounty.png"}]
